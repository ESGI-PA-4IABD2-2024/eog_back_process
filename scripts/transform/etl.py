    ...
    for route_schedule in data['route_schedules']:
        ...
        for circulations in route_schedule['table']['headers']:
            ...
            if id_circulation not in circulations:
                dic_circulations[id_circulation] = Circulation(id_circulation, circulation_type, quai, name_train)
            ...
            for row in route_schedule['table']['rows']:
                ...
                for dt in row['date_times']:
                    if dt['links'] != [] and dt['links'][0][
                        'id'] == id_circulation:  # Vérifier que l'arrêt correspond à la circulation en cours
                        h_stop = dt['date_time']

                        if h_stop != '':
                            dic_circulations[id_circulation].trajet.append(id_circulation, name_gare, h_stop, quai)
