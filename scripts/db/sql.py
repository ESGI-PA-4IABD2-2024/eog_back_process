        ...
        for stop in iter_stops(circulation.trajet.head):
            ...
            if gare:
                quai = (
                    ...
                    .filter_by(
                        id_gare=gare.id,
                        ligne_quai=circulation.quai,
                        type=circulation.circulation_type,
                    )
                    .first()
                )
                if not quai:
...
def insert_trajets(circulations):
    for circulation in circulations.values():
        ...
        while current_stop and current_stop.next_gare:
            ...
            gare_arrivee = (
                session.query(Gare).filter_by(name=current_stop.next_gare.name_gare).first()
            )
            quai_depart = (
                session.query(Quai)
                .filter_by(
                    id_gare=gare_depart.id,
                    ligne_quai=circulation.quai,
                    type=circulation.circulation_type,
                )
                .first()
            ...
            new_trajet = Trajet(
                id_train=session.query(Circulation)
                .filter_by(id_train=circulation.id_circulation)
                .first()
                .id,
